...
def test_summary_stats():
    ...
    sample_dataframe = pd.read_csv(
        "python_script/graduate_program_recommendation_data.csv"
    )
    ...
    assert (
        list(summary_stats.columns) == expected_columns
    ...
    assert len(summary_stats) == len(
        numeric_columns
    ), "The number of rows in summary stats does not match numeric columns."
